...
@Component({
   ...
   styleUrls: ['./parada.component.scss']
})
...
export class ParadaComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: ParadaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: Parada): boolean {
      return true;
   }
   ...
   crearEntidad(): Parada {
      const nuevoParada = new Parada();
      nuevoParada.id = 0;
      return nuevoParada;
   }

   add(entidadNueva: Parada): void {
   ...
   update(entidadParaActualizar: Parada): void {
   ...
   delete(entidadParaBorrar: Parada): void {
   ...
   onSelect(entidadActual: Parada): void {
