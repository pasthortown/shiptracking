...
export class ExpresionComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: ExpresionService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: Expresion): boolean {
      return true;
   }
   ...
   crearEntidad(): Expresion {
      const nuevoExpresion = new Expresion();
      nuevoExpresion.id = 0;
      return nuevoExpresion;
   }

   add(entidadNueva: Expresion): void {
   ...
   update(entidadParaActualizar: Expresion): void {
   ...
   delete(entidadParaBorrar: Expresion): void {
   ...
   onSelect(entidadActual: Expresion): void {
