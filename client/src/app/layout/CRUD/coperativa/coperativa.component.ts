...
@Component({
   selector: 'app-coperativa',
   templateUrl: './coperativa.component.html',
   styleUrls: ['./coperativa.component.scss']
})

export class CoperativaComponent implements OnInit {

   busy: Promise<any>;
   entidades: Coperativa[];
   entidadSeleccionada: Coperativa;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CoperativaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: Coperativa): boolean {
      return true;
   }
   ...
   crearEntidad(): Coperativa {
      const nuevoCoperativa = new Coperativa();
      nuevoCoperativa.id = 0;
      return nuevoCoperativa;
   }

   add(entidadNueva: Coperativa): void {
   ...
   update(entidadParaActualizar: Coperativa): void {
   ...
   delete(entidadParaBorrar: Coperativa): void {
   ...
   onSelect(entidadActual: Coperativa): void {
