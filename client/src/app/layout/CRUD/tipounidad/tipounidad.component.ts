...
export class TipoUnidadComponent implements OnInit {
   ...
   busy: Promise<any>;
   entidades: TipoUnidad[];
   entidadSeleccionada: TipoUnidad;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoUnidadService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: TipoUnidad): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoUnidad {
      const nuevoTipoUnidad = new TipoUnidad();
      nuevoTipoUnidad.id = 0;
      return nuevoTipoUnidad;
   }

   add(entidadNueva: TipoUnidad): void {
   ...
   update(entidadParaActualizar: TipoUnidad): void {
   ...
   delete(entidadParaBorrar: TipoUnidad): void {
   ...
   onSelect(entidadActual: TipoUnidad): void {
