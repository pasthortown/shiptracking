...
export class AdjuntoComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: AdjuntoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: Adjunto): boolean {
      return true;
   }
   ...
   crearEntidad(): Adjunto {
      const nuevoAdjunto = new Adjunto();
      nuevoAdjunto.id = 0;
      return nuevoAdjunto;
   }

   add(entidadNueva: Adjunto): void {
   ...
   update(entidadParaActualizar: Adjunto): void {
   ...
   delete(entidadParaBorrar: Adjunto): void {
   ...
   onSelect(entidadActual: Adjunto): void {
